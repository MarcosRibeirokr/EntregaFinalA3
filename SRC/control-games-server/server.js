...
app.delete('/usuarios/:nickname', (req, res) => {
    ...
    // Deletar usuario 
    db.run('DELETE FROM usuarios WHERE nickname = ?', [nickname], function (err) {
        console.log('usuario excluido no backend', nickname)
...
app.delete('/plataformas/:nomePlataforma/:usuario', (req, res) => {
    ...
    db.run('SELECT nickname FROM usuarios WHERE nickname = ?', [usuario], (err) => {
        ...
        if (usuario === 'admin') {
            db.get('SELECT COUNT(*) as count FROM jogos WHERE plataforma = ?', [nomePlataforma], (err, row) => {
                if (err) {
                    return res.status(500).json({ error: err.message });
                }
    
                if (row.count > 0) {
                    // Se existirem jogos associados, não exclua a plataforma
                    return res.status(400).json({ error: 'Existem jogos associados a esta plataforma. Não é possível excluir.' });
                }
    
                // Excluir a plataforma após verificar que não há jogos associados
                db.run('DELETE FROM plataformas WHERE nomePlataforma = ?', [nomePlataforma], function (err) {
                    if (err) {
                        return res.status(500).json({ error: err.message });
                    }
